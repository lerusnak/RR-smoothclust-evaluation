...
*X-Y  Spatial Plot - SM*
```{r}
colData(spe)$SM <- kern.02.km6.sm$n_discordant
plotSpots(spe, annotate = "SM", 
          pal = c("gray", "red"),
          point_size = 1)
```
...
```{r}
gridExtra::grid.arrange(
  (xyplot_uni_km4 + labs(title="Spatial - uniform")),
  (xyplot_nonspac_km8 + labs(title="Non-spatial")),
  ncol=2)
```
...
```{r}
gridExtra::grid.arrange(
  (xyplot_kern.08_km6 + labs(title="Spatial - kernel (0.08)")),
  (xyplot_nonspac_km8 + labs(title="Non-spatial")),
  ncol=2)
```
